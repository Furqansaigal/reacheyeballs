...
import { ArrowRight, Check, ChevronDown, ChevronLeft, ChevronRight, Menu, X, CalendarDays, MapPin, Target, Sparkles, ShieldCheck, Clock3, CircleCheck } from 'lucide-react';
...
function getBookingDays(leadDays: number) {
  const date = new Date();
  date.setHours(12, 0, 0, 0);
  date.setDate(date.getDate() + leadDays);

  const dates: { day: string; date: number; label: string }[] = [];
...
export default function Home() {
  ...
  const [selectedTime, setSelectedTime] = useState<string>(content.booking.timeSlots[0]);
  const [datePage, setDatePage] = useState(0);
  const heroVideoRef = useRef<HTMLVideoElement>(null);
  const close = () => setMenuOpen(false);
  const bookingDays = getBookingDays(content.booking.leadDays + datePage * 3);
  const moveDatePage = (direction: -1 | 1) => {
    setDatePage(page => Math.max(0, page + direction));
    setSelectedDay(0);
  };
  const moveTime = (direction: -1 | 1) => {
    const slots = content.booking.timeSlots;
    const currentIndex = slots.indexOf(selectedTime as typeof slots[number]);
    setSelectedTime(slots[(currentIndex + direction + slots.length) % slots.length]);
  };
  ...
  return <main id="top" className="overflow-x-hidden pt-[68px] sm:pt-[76px]">
    ...
    <section className="hero-shell relative border-b border-[#dce5f3] bg-white">
      <div className="grid-glow absolute inset-0 opacity-70" />
      <div className="hero-ambient" aria-hidden="true"><span className="hero-orb hero-orb-copy" /><span className="hero-orb hero-orb-secondary" /></div>
      <div className="relative z-10 mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:gap-12 sm:px-8 sm:py-16 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,.92fr)] xl:items-start xl:py-20">
        <Reveal className="min-w-0">
          <Eyebrow>LEAD GENERATION FOR UK WINDOW &amp; DOOR COMPANIES</Eyebrow>
          <h1 className="hero-title max-w-3xl font-black leading-[.97] tracking-[-.07em] text-[#07101f]">Stop Chasing Leads.<br />Start Closing <span className="text-[#2564d9]">Qualified</span><br />Appointments.</h1>
          <p className="mt-5 max-w-xl text-base leading-7 text-[#526078] sm:text-lg sm:leading-8">We help window &amp; door companies generate exclusive, pre-qualified homeowner enquiries and book them directly into your calendar.</p>
          <p className="mt-3 max-w-xl text-sm leading-6 text-[#526078]">No shared Bark leads. No endless cold calling. No wasting hours chasing people who were never ready to buy.</p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row"><Button /><Button href="#how-it-works" secondary>SEE HOW IT WORKS</Button></div>
          <ul className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-[#314059]">{['Exclusive Appointments', 'Qualified Leads', 'Done-For-You Service'].map(item => <li key={item} className="flex items-center gap-2"><Check size={16} className="text-[#2564d9]" />{item}</li>)}</ul>
          <div className="mt-7 flex items-center gap-3 text-xs font-semibold text-[#526078]"><span className="flex -space-x-2">{[0, 1, 2].map(i => <span key={i} className="grid h-7 w-7 place-items-center rounded-full border-2 border-white bg-[#dbe9ff] text-[9px] font-black text-[#2564d9]">{['UK', '✓', '24'][i]}</span>)}</span><span>Built exclusively for UK installers</span></div>
        </Reveal>
        <Reveal delay={120} className="relative mx-auto w-full max-w-[280px] sm:max-w-[300px] xl:mt-2">
    ...
    <section className="border-b border-[#dce5f3] bg-[#eef4ff]"><div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-5 py-6 text-center sm:px-8 md:flex-row md:text-left"><div><p className="text-[11px] font-bold tracking-[.15em] text-[#2564d9]">BUILT FOR WINDOW &amp; DOOR COMPANIES</p><p className="mt-1 text-sm font-semibold text-[#35425a]">Not another generalist marketing agency.</p></div><div className="flex items-center gap-2 text-xs font-bold tracking-[.12em] text-[#526078]"><span className="status-dot h-2 w-2 rounded-full bg-[#18a957]" />UK WINDOW &amp; DOOR LEAD GENERATION SPECIALISTS</div></div></section>

    <section className="mx-auto max-w-7xl px-5 py-24 sm:px-8"><Reveal className="max-w-2xl"><Eyebrow>SOUND FAMILIAR?</Eyebrow><h2 className="text-4xl font-black leading-[.98] tracking-[-.06em] sm:text-5xl">Your Problem Isn&apos;t Selling Windows.<br /><span className="text-[#2564d9]">It&apos;s Getting the Right Opportunities.</span></h2></Reveal><div className="mt-12 grid gap-px border border-[#dce5f3] bg-[#dce5f3] md:grid-cols-2 xl:grid-cols-4">{content.painPoints.map(([num, title, text], i) => <Reveal key={num} delay={i * 80}><article className="group h-full bg-white p-7 transition duration-300 hover:-translate-y-1 hover:bg-[#07101f] hover:shadow-xl"><p className="text-xs font-black tracking-[.15em] text-[#2564d9]">{num}</p><h3 className="mt-12 text-xl font-black tracking-[-.04em] group-hover:text-white">{title}</h3><p className="mt-3 text-sm leading-6 text-[#63718a] group-hover:text-[#b5c2d8]">{text}</p></article></Reveal>)}</div><p className="mt-8 text-lg font-bold">Reach Eyeballs was built to change that.</p></section>

    <section id="difference" className="blueprint relative overflow-hidden px-5 py-24 sm:px-8"><div className="absolute inset-0 bg-[linear-gradient(115deg,transparent_30%,rgba(37,100,217,.14))]" /><div className="relative mx-auto max-w-7xl"><Reveal className="max-w-3xl"><Eyebrow light>THE REACH EYEBALLS DIFFERENCE</Eyebrow><h2 className="text-4xl font-black leading-[.98] tracking-[-.06em] text-white sm:text-5xl">Exclusive Opportunities.<br /><span className="text-[#6da2ff]">Qualified Homeowners.</span><br />Appointments Already Booked.</h2><p className="mt-6 max-w-2xl text-base leading-7 text-[#c1cce0]">Reach Eyeballs targets homeowners actively considering window and door replacement within your service area. Instead of simply sending contact details, we help qualify prospects and book suitable opportunities directly into your calendar.</p></Reveal><div className="mt-12 grid gap-px border border-white/10 bg-white/10 md:grid-cols-2 xl:grid-cols-3">{content.solutions.map(([title, text], i) => <Reveal key={title} delay={(i % 3) * 80}><article className="group h-full bg-[#0d1829] p-7 transition duration-300 hover:-translate-y-1 hover:bg-[#122747]"><span className="grid h-9 w-9 place-items-center bg-[#2564d9] text-xs font-black text-white transition group-hover:scale-110">0{i + 1}</span><h3 className="mt-8 text-xl font-black tracking-[-.04em] text-white">{title}</h3><p className="mt-3 text-sm leading-6 text-[#b9c6db]">{text}</p></article></Reveal>)}</div></div></section>

    <section id="how-it-works" className="mx-auto max-w-7xl px-5 py-24 sm:px-8"><Reveal className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><Eyebrow>HOW IT WORKS</Eyebrow><h2 className="text-4xl font-black tracking-[-.06em] sm:text-5xl">From Click to<br /><span className="text-[#2564d9]">Qualified Appointment.</span></h2></div><p className="max-w-sm text-sm leading-6 text-[#63718a]">A clear, sales-focused workflow from local homeowner targeting to your live sales calendar.</p></Reveal><div className="mt-14 grid gap-8 md:grid-cols-4">{content.process.map(([num, title, text], i) => <Reveal key={num} delay={i * 100}><article className="relative border-t-2 border-[#2564d9] pt-5">{i < 3 && <span className="absolute left-10 right-[-2rem] top-[-1px] hidden h-px bg-[#dce5f3] md:block" />}<p className="text-xs font-black tracking-[.15em] text-[#2564d9]">{num}</p><h3 className="mt-8 text-2xl font-black tracking-[-.04em]">{title}</h3><p className="mt-3 text-sm leading-6 text-[#63718a]">{text}</p></article></Reveal>)}</div></section>

    <section id="results" className="blueprint px-5 py-24 sm:px-8"><div className="mx-auto max-w-7xl"><Eyebrow light>RESULTS THAT MATTER</Eyebrow><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]"><Reveal><h2 className="text-4xl font-black leading-[.98] tracking-[-.06em] text-white sm:text-5xl">We Don&apos;t Sell Clicks.<br /><span className="text-[#6da2ff]">We Build Sales Opportunities.</span></h2><p className="mt-6 max-w-md text-base leading-7 text-[#b9c6db]">The goal isn&apos;t vanity metrics. It&apos;s more conversations with homeowners who are genuinely considering replacing their windows or doors.</p><div className="mt-8 flex items-center gap-3 text-sm text-[#c1cce0]"><ShieldCheck className="text-[#6da2ff]" size={19} /><span>Built around exclusive local opportunities.</span></div></Reveal><div className="grid gap-px border border-white/10 bg-white/10 sm:grid-cols-2">{content.stats.map((stat, i) => <Reveal key={stat.label} delay={i * 80}><div className="h-full bg-[#0d1829] p-7 transition duration-300 hover:bg-[#122747]"><AnimatedValue value={stat.value} /><p className="mt-4 text-sm font-bold text-[#78aaff]">{stat.label}</p><p className="mt-1 text-xs text-[#aebbd0]">{stat.note}</p></div></Reveal>)}</div></div></div></section>
    ...
    <section className="px-4 py-16 sm:px-8 sm:py-24"><div className="blueprint mx-auto max-w-7xl overflow-hidden p-6 sm:p-14"><Eyebrow light>YOUR NEXT STEP</Eyebrow><div className="grid min-w-0 gap-8 lg:grid-cols-[1.3fr_.7fr] lg:items-end"><div className="min-w-0"><h2 className="cta-title max-w-full font-black leading-[.97] tracking-[-.065em] text-white">Ready to Fill Your Calendar With <span className="text-[#6da2ff]">Better Opportunities?</span></h2><p className="mt-5 max-w-xl text-base leading-7 text-[#c1cce0]">Book a free 30-minute strategy call and discover how Reach Eyeballs can help generate qualified window and door appointments in your area.</p></div><div className="min-w-0 lg:text-right"><Button className="w-full sm:w-auto"><span className="sm:hidden">BOOK MY FREE CALL</span><span className="hidden sm:inline">BOOK MY FREE STRATEGY CALL</span></Button><ul className="mt-5 grid gap-1.5 text-left text-xs leading-5 text-[#c1cce0] lg:justify-items-end lg:text-right"><li>✓ Free 30-minute consultation</li><li>✓ No obligation</li><li>✓ Custom strategy for your area</li></ul></div></div></div></section>

    <section id="booking" className="border-y border-[#dce5f3] bg-white px-5 py-24 sm:px-8">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.85fr_1.15fr]">
        <Reveal><Eyebrow>BOOK A FREE STRATEGY CALL</Eyebrow><h2 className="text-4xl font-black tracking-[-.06em] sm:text-5xl">Let&apos;s See If Your Area Is Available.</h2><p className="mt-6 max-w-md text-base leading-7 text-[#526078]">Choose a convenient time for a free strategy call. We&apos;ll talk through your area, current pipeline and whether our approach is right for you.</p><p className="mt-8 text-sm font-bold text-[#2564d9]">We limit the number of companies we work with in each area.</p></Reveal>
        <Reveal delay={120}>
          <div className="booking-card border border-[#dce5f3] bg-[#f6f8fc] p-5 sm:p-8">
            <div className="flex items-center gap-3 border-b border-[#dce5f3] pb-5"><CalendarDays className="shrink-0 text-[#2564d9]" /><div><p className="font-black">Book your free strategy call</p><p className="text-xs text-[#63718a]">30 minutes · Online meeting</p></div></div>

            <div className="mt-6 flex items-center justify-between sm:hidden"><button onClick={() => moveDatePage(-1)} disabled={datePage === 0} aria-label="Show previous available dates" className="grid h-9 w-9 place-items-center rounded-full border border-[#dce5f3] bg-white text-[#2564d9] disabled:opacity-35"><ChevronLeft size={17} /></button><p className="text-[10px] font-bold tracking-[.12em] text-[#63718a]">CHOOSE A DATE</p><button onClick={() => moveDatePage(1)} aria-label="Show next available dates" className="grid h-9 w-9 place-items-center rounded-full border border-[#dce5f3] bg-white text-[#2564d9]"><ChevronRight size={17} /></button></div>
            <div className="mt-2 flex items-stretch gap-2 sm:mt-6"><button onClick={() => moveDatePage(-1)} disabled={datePage === 0} aria-label="Show previous available dates" className="hidden w-10 shrink-0 place-items-center border border-[#dce5f3] bg-white text-[#2564d9] transition hover:border-[#2564d9] disabled:cursor-not-allowed disabled:opacity-35 sm:grid"><ChevronLeft size={18} /></button><div className="grid min-w-0 flex-1 grid-cols-3 gap-1.5 sm:gap-2">{bookingDays.map((day, i) => <button onClick={() => setSelectedDay(i)} key={day.label} className={`border p-2 text-left transition sm:p-3 ${selectedDay === i ? 'border-[#2564d9] bg-[#2564d9] text-white shadow-[0_8px_18px_rgba(37,100,217,.22)]' : 'border-[#dce5f3] bg-white hover:border-[#2564d9]'}`}><span className="block text-[9px] font-bold sm:text-[10px]">{day.day}</span><span className="mt-1 block text-lg font-black">{day.date}</span></button>)}</div><button onClick={() => moveDatePage(1)} aria-label="Show next available dates" className="hidden w-10 shrink-0 place-items-center border border-[#dce5f3] bg-white text-[#2564d9] transition hover:border-[#2564d9] hover:bg-[#eaf2ff] sm:grid"><ChevronRight size={18} /></button></div>

            <div className="mt-5 flex items-center justify-between sm:hidden"><button onClick={() => moveTime(-1)} aria-label="Select previous time" className="grid h-9 w-9 place-items-center rounded-full border border-[#dce5f3] bg-white text-[#2564d9]"><ChevronLeft size={17} /></button><p className="text-[10px] font-bold tracking-[.12em] text-[#63718a]">CHOOSE A TIME</p><button onClick={() => moveTime(1)} aria-label="Select next time" className="grid h-9 w-9 place-items-center rounded-full border border-[#dce5f3] bg-white text-[#2564d9]"><ChevronRight size={17} /></button></div>
            <div className="mt-2 flex items-stretch gap-2 sm:mt-5"><button onClick={() => moveTime(-1)} aria-label="Select previous time" className="hidden w-10 shrink-0 place-items-center border border-[#dce5f3] bg-white text-[#2564d9] transition hover:border-[#2564d9] hover:bg-[#eaf2ff] sm:grid"><ChevronLeft size={18} /></button><div className="grid min-w-0 flex-1 grid-cols-3 gap-1.5 sm:gap-2">{content.booking.timeSlots.map(time => <button onClick={() => setSelectedTime(time)} key={time} className={`border px-1 py-3 text-[10px] font-bold transition sm:px-3 sm:text-xs ${selectedTime === time ? 'border-[#2564d9] bg-[#eaf2ff] text-[#174eb9]' : 'border-[#dce5f3] bg-white hover:border-[#2564d9] hover:text-[#2564d9]'}`}>{time}</button>)}</div><button onClick={() => moveTime(1)} aria-label="Select next time" className="hidden w-10 shrink-0 place-items-center border border-[#dce5f3] bg-white text-[#2564d9] transition hover:border-[#2564d9] hover:bg-[#eaf2ff] sm:grid"><ChevronRight size={18} /></button></div>

            <a href={`mailto:${content.email}?subject=Free%20strategy%20call%20-%20${encodeURIComponent(bookingDays[selectedDay].label)}%20at%20${encodeURIComponent(selectedTime)}`} className="mt-6 flex items-center justify-center gap-2 bg-[#2564d9] px-4 py-4 text-xs font-bold tracking-[.07em] text-white transition hover:bg-[#174eb9] hover:shadow-[0_14px_28px_rgba(37,100,217,.25)]">REQUEST {selectedTime} SLOT <ArrowRight size={15} /></a><p className="mt-4 flex items-center justify-center gap-1.5 text-center text-[11px] text-[#63718a]"><Clock3 size={12} />You&apos;ll receive a confirmation by email.</p>
          </div>
        </Reveal>
      </div>
    </section>
